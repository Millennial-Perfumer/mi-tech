...
### [IDE-010] Automated B2B Proforma WhatsApp Follow-Up & Negotiation Loop
* **Added On**: 2024-07-07
* **Target Audience**: B2B Clients, Sales Representatives
* **3L Growth Vector**: Increase Average Order Value (AOV) & Boost Traffic-to-Customer Conversion
* **Customer Value Proposition**: Provides a seamless, conversational way for B2B buyers to receive, review, and negotiate proforma invoices directly on WhatsApp. This reduces friction and response time, ensuring they can finalize their high-value orders with a personalized touch.
* **Business Growth & Profit Impact**: B2B transactions inherently carry higher AOV than retail. Proforma invoices often sit in "SENT" state due to friction in traditional email communication or lack of timely follow-ups. By automating WhatsApp follow-ups for proformas in the "SENT" state and enabling an interactive negotiation loop, we can significantly accelerate the conversion of these high-value draft orders to "ACCEPTED" and "CONVERTED_TO_INVOICE" states, directly impacting our monthly revenue targets with high margin.
* **Technical Complexity**: Medium
* **Description**: Implement a scheduled job that monitors the `B2BProformaInvoice` table for invoices in the `SENT` status that have been inactive for more than 48 hours. Integrate with the `communication` (SMM) module to dispatch an automated WhatsApp message to the customer's `CustomerPhone` (via the `NormalizePhone` util for valid E.164 formatting). The message will include a secure link to view the proforma and interactive buttons (e.g., "Accept", "Request Revision", "Chat with Agent"). If they select "Chat with Agent", it delegates the context to the `AgentService` to handle initial negotiations, saving sales representative time and nudging the buyer toward conversion.
