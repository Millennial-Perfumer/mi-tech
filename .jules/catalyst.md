...
### [IDE-007] Automated Post-Purchase Replenishment Sequences
* **Added On**: 2026-06-22
* **Target Audience**: End Customers
* **3L Growth Vector**: Increase Purchase Frequency (LTV Boost)
* **Customer Value Proposition**: Anticipates exactly when a customer is likely to run out of their consumable items (e.g., specific perfumes) and sends a timely, personalized reminder to restock, creating a magical "they know what I need" customer experience.
* **Business Growth & Profit Impact**: Consumable products have a natural lifecycle. By calculating average depletion times and proactively messaging customers via WhatsApp right when they need a refill, we can secure repeat orders that might otherwise go to competitors or be forgotten. This directly drives higher Customer Lifetime Value (LTV) and recurring revenue streams without additional ad spend, significantly accelerating our path to 3L/month while optimizing for high-margin repeat business.
* **Technical Complexity**: Medium
* **Description**: Implement a predictive model in the `planner` or `ai` domain that calculates the average days between repeat purchases for specific SKUs or categories (e.g., 30ml vs 100ml perfume bottles). Create a scheduled background job that cross-references past orders and tags customers reaching the 80% depletion point. Integrate this trigger with the `communication` module (SMM Hub) to auto-dispatch a WhatsApp template containing a one-click re-order link for their specific item.
