...
## 2026-06-16 - [Fix Timing Attack in Shopify Webhook Verification]\n**Vulnerability:** The Shopify webhook handler verified the HMAC signature by comparing the base64-encoded hashes using the '==' operator. This standard string comparison operator returns immediately upon finding the first mismatched byte, exposing the verification process to timing attacks where an attacker could theoretically guess the signature byte-by-byte.\n**Learning:** The '==' operator in Go, like many other languages, compares strings byte-by-byte and exits early. This is not constant-time.\n**Prevention:** When comparing cryptographic signatures or hashes, always use constant-time comparison functions. In Go, use 'subtle.ConstantTimeCompare' or 'hmac.Equal' (by converting the strings to byte slices).
