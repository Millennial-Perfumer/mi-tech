...
## 2025-02-27 - [Systemic DoS Vulnerability in Handlers via Unbounded Request Bodies]
**Vulnerability:** Numerous HTTP handlers across various modules (including order, production, inventory, ai, feedback, and support) read the HTTP request body using `json.NewDecoder(r.Body)` or `io.ReadAll(r.Body)` without first restricting the maximum payload size, presenting a critical unmitigated Denial-of-Service (DoS) memory exhaustion risk.
**Learning:** Even internal or authenticated routes can be vulnerable to resource exhaustion. The Go standard library does not automatically limit request bodies, so manual constraints must be applied before decoding in every single handler that accepts a body.
**Prevention:** Always bound request body sizes using `r.Body = http.MaxBytesReader(w, r.Body, 1048576)` (1MB limit) at the start of HTTP POST/PUT/PATCH handlers, before calling `io.ReadAll` or decoding JSON.
