...
## 2025-02-13 - [Denial of Service via Default HTTP Client]
**Vulnerability:** External API integration (`MetaClient`) was using `http.DefaultClient.Do(req)`, which has no timeout by default.
**Learning:** `http.DefaultClient` can lead to resource exhaustion/Denial of Service (DoS) attacks if the upstream API hangs or responds very slowly, tying up goroutines and network sockets indefinitely. This is a common and critical security issue in Go network integrations.
**Prevention:** Always initialize and inject a custom `&http.Client{}` with an explicit `Timeout` (e.g., `Timeout: 30 * time.Second`) instead of using `http.DefaultClient` for making outbound HTTP calls.
