...
## 2026-06-29 - [Interactive Sorting Table Headers Accessibility]
**Learning:** Found instances where `<th>` elements contained `onClick` handlers and dynamic text indicators (like ↑ or ↓) for sorting, but lacked proper accessibility attributes. Keyboard users could not access the sort function via Tab, and screen readers did not announce the headers as interactive. Relying on an internal `aria-label` overrides the text for screen readers, hiding the actual column name.
**Action:** When implementing or fixing sortable table headers, always wrap the header text in a `<button type="button">` (styled to remove default appearance with `background: 'none'`, `border: 'none'`, `font: 'inherit'`) and apply the `aria-sort` attribute directly to the parent `<th>` element (values: `"ascending"`, `"descending"`, or `"none"`).
