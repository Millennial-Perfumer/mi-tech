...
## 2026-06-25 - [WhatsApp Chat Component Accessibility]
**Learning:** Found several utility buttons (Clear Search, Mode Toggles, Upload Media, Send Message) in `WhatsAppChat.tsx` lacking `type="button"`, increasing the risk of accidental form submissions. Additionally, the icon-only upload and send buttons lacked explicit `aria-label`s, negatively impacting screen reader users.
**Action:** Always add `type="button"` defensively to action buttons within complex interactive components and ensure `aria-label` or `title` is applied to all icon-only interactive controls.
