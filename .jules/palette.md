...
## 2024-07-12 - Accessibility in WhatsAppChat
**Learning:** Icon-only buttons within conversational interfaces (like WhatsAppChat) frequently miss explicit ARIA attributes and `type="button"`. Because these complex UI views often don't reside strictly within `<form>` tags, omitting `type="button"` can trigger accidental browser defaults. Also, custom multi-choice toggles (like Human/Auto mode) must utilize `aria-pressed` to inform screen readers of the active state.
**Action:** Always add `type="button"` to any non-submit button. Ensure icon-only buttons include `aria-label` (and often `title` for visual users). For toggle groups, track active states using `aria-pressed`. For decorative SVGs inside these buttons, inject `aria-hidden="true"` to prevent redundant screen reader announcements.
