...
## 2026-06-25 - [Parallelize Amazon Order Item Fetching]
**Learning:** Fetching `amazonClient.GetOrderItems` sequentially within the main `AmazonOrderPoller.SyncOrders` loop causes an O(N) network latency bottleneck. Every external SP-API call pauses the loop.
**Action:** When iterating over a batch of external entities that require further network lookups, use `golang.org/x/sync/errgroup` to fetch the data concurrently into a thread-safe map guarded by a `sync.Mutex` before entering the main processing loop. Limit concurrency (e.g. `eg.SetLimit(5)`) to respect Amazon's strict rate limits.
