...
## 2026-06-25 - [Parallelize Amazon Orders Polling]
**Learning:** Iterating through fetched orders sequentially and making external API calls (e.g. `amazonClient.GetOrderItems(amazonOrderID)`) inside the loop creates an O(N) performance bottleneck.
**Action:** Use `golang.org/x/sync/errgroup` to parallelize the requests, limit concurrency to `5` to prevent overloading the database/API, capture the loop variable properly (`ao := ao`), and aggregate safely. Be sure to use `sync/atomic` if incrementing sequences across concurrent routines.
