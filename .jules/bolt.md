...
## 2026-03-27 - [Multi-Column Batch Lookups with Tuple IN]
**Learning:** When performing batch lookups based on composite keys (e.g., platform and external_sku), GORM's `Where("(col1, col2) IN ?", pairs)` with a slice of slices (`[][]interface{}`) is significantly more efficient than individual lookups or multiple `OR` clauses. This allows the database to use appropriate indexes and reduces round-trips to O(1).
**Action:** Use tuple `IN` queries for batch lookups involving composite keys to avoid N+1 query patterns.
