.PHONY: install install-frontend install-frontend-legacy install-frontend-feedback install-shop-chat-agent install-backend run frontend frontend-legacy frontend-feedback backend build build-frontend build-frontend-legacy build-frontend-feedback build-shop-chat-agent build-backend clean db-up db-down
...
install: install-frontend install-frontend-legacy install-frontend-feedback install-shop-chat-agent install-backend

install-frontend:
	cd frontend && npm ci --legacy-peer-deps

install-frontend-legacy:
	cd frontend-legacy && npm ci --legacy-peer-deps
...
frontend-legacy:
	cd frontend-legacy && npm run dev -- --port 5174
...
build: build-frontend build-frontend-legacy build-frontend-feedback build-shop-chat-agent build-backend

build-frontend:
	cd frontend && npm run build

build-frontend-legacy:
	cd frontend-legacy && npm run build
