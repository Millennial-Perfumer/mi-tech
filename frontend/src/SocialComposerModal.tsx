  ...
  const handlePost = async () => {
    ...
    try {
      for (const platform of selectedPlatforms) {
        const resp = await fetchWithAuth(`${API_BASE}/api/marketing/smm/post`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            platform,
            message: content, caption: content, text: content,
            image_url: 'https://placeholder.com/image.jpg'
          })
        });
        if (!resp.ok) throw new Error(`Failed on ${platform}`);
      }
    ...
    <div className="modal-overlay" style={{ zIndex: 2000 }} onClick={onClose}>
      <div className="premium-modal glass-card-premium" style={{ maxWidth: '600px', position: 'relative' }} onClick={e => e.stopPropagation()}>
        <button
          ...
          style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', border: 'none', background: 'none', cursor: 'pointer', fontSize: '1.2rem', color: 'var(--text-secondary)' }}
        >✕</button>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <textarea 
            ...
            style={{
              width: '100%', height: '150px', padding: '1rem', borderRadius: '12px', background: 'var(--bg-input)',
              border: '1px solid var(--border-color)', color: 'var(--text-primary)', fontFamily: 'inherit', resize: 'none', outline: 'none'
            }}
          />
          <div style={{ textAlign: 'right', fontSize: '0.75rem', color: 'var(--text-tertiary)', marginTop: '0.5rem', fontWeight: 600 }} aria-live="polite">
