...
export const MarketingDashboard: React.FC<MarketingDashboardProps> = ({ fetchWithAuth, userRole, onNavigate }) => {
  ...
  const getExceptions = () => {
    ...
    if (roas < 1.5 && roas > 0) list.push({ type: 'warning', msg: 'Low ROAS Alert: Check performance.', color: 'var(--status-warning)' });
    if (freq > 3.0) list.push({ type: 'danger', msg: 'High Frequency detected: Fatigue alert.', color: 'var(--status-danger)' });
    ...
    <div className="tab-content-fade" style={{ padding: '0 1rem' }}>
      ...
      <div className="glass-island-premium" style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ background: 'var(--nav-active-bg)', padding: '12px', borderRadius: '16px' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="var(--accent-color)"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
          </div>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.5px' }}>
              Ads Intelligence {userRole === 'admin' && <span style={{ fontSize: '0.65rem', background: 'var(--status-active-bg)', color: 'var(--status-active)', padding: '2px 8px', borderRadius: '6px', verticalAlign: 'middle', marginLeft: '8px' }}>Admin</span>}
            </h2>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '2px' }}>
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--status-active)', fontWeight: 700 }}>Real-time</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <button className="btn-secondary" onClick={() => onNavigate?.('settings')} style={{ padding: '0.65rem 1.25rem', borderRadius: '14px', fontSize: '0.85rem' }}>
            API Settings
          </button>
        </div>
      </div>
      ...
      {isLoading && !metaData.metrics.spend ? (
          <div style={{ padding: '8rem', textAlign: 'center' }}>
            <div className="spinner" style={{ margin: '0 auto 2rem', width: '50px', height: '50px', borderTopColor: 'var(--accent-color)' }}></div>
            <p style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--text-secondary)' }}>Aggregating Meta Performance...</p>
          </div>
      ) : error ? (
        <div className="glass-card" style={{ padding: '4rem', textAlign: 'center', background: 'var(--status-danger-bg)', border: '1px solid var(--status-danger)' }}>
          <div style={{ color: 'var(--status-danger)', marginBottom: '1.5rem' }}>
        ...
        <>
          ...
          {parseFloat(metaData.metrics.spend || '0') === 0 && !isLoading && (
            <div style={{ padding: '1rem 1.5rem', borderRadius: '16px', background: 'var(--status-active-bg)', border: '1px solid var(--status-active)', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--status-active)" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
              <span style={{ fontSize: '0.85rem', color: 'var(--text-primary)', fontWeight: 600 }}>No spend detected for this period. Try searching the last 30 days.</span>
            </div>
          )}
          ...
          <div className="metrics-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            <MetricCard label="Spend" value={formatCurrency(currentMetrics.spend)} sub="Current Period" color="var(--accent-color)" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><path d="M16 8h-6a2 2 0 1 0 0 4h4a2 2 0 1 1 0 4H8"/><path d="M12 18V6"/></svg>} />
            <MetricCard label="ROAS" value={`${parseFloat(currentMetrics.purchase_roas_val || '0').toFixed(2)}x`} sub="Return on spend" color="var(--status-active)" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>} />
            <MetricCard label="Conv." value={formatNumber(currentMetrics.conversions || 0)} sub="Total results" color="var(--status-active)" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>} />
            <MetricCard label="Freq." value={parseFloat(currentMetrics.frequency || '0').toFixed(1)} sub="Ad Saturation" color="var(--status-warning)" icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>} />
          </div>

          {/* Drill-down Intelligence Container */}
          <div className="glass-island-premium" style={{ borderRadius: '24px', overflow: 'hidden' }}>
            <div style={{ padding: '1.5rem 1.8rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <table className="premium-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'var(--bg-hover)' }}>
                    <th style={{ textAlign: 'left', padding: '1.2rem 1.8rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid var(--border-color)' }}>
                      {viewLevel === 'CAMPAIGNS' ? 'Campaign Details' : viewLevel === 'AD_SETS' ? 'Ad Set Name' : 'Creative & Preview'}
                    </th>
                    <th style={{ textAlign: 'left', padding: '1.2rem 1.8rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid var(--border-color)' }}>Status</th>
                    <th style={{ textAlign: 'left', padding: '1.2rem 1.8rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid var(--border-color)' }}>
                      {viewLevel === 'CAMPAIGNS' ? 'Objective' : viewLevel === 'AD_SETS' ? 'Budget' : 'Performance'}
                    </th>
                    {viewLevel === 'ADS' && (
                      <>
                        <th style={{ textAlign: 'left', padding: '1.2rem 1.8rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid var(--border-color)' }}>Reach</th>
                        <th style={{ textAlign: 'left', padding: '1.2rem 1.8rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid var(--border-color)' }}>Freq.</th>
                        <th style={{ textAlign: 'left', padding: '1.2rem 1.8rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid var(--border-color)' }}>Clicks</th>
                      </>
                    )}
                    <th style={{ textAlign: 'right', padding: '1.2rem 1.8rem', color: 'var(--text-tertiary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', borderBottom: '1px solid var(--border-color)' }}>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {viewLevel === 'CAMPAIGNS' && metaData.campaigns.length > 0 ? metaData.campaigns.map(camp => {
                    ...
                    return (
                      <tr key={camp.id} className="hover-row" style={{ cursor: 'pointer' }}>
                        <td onClick={() => fetchAdSets(camp.id, camp.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '1rem', marginBottom: '2px' }}>{camp.name}</div>
                          <div style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>ID: {camp.id}</div>
                        </td>
                        <td onClick={() => fetchAdSets(camp.id, camp.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          <span style={{ fontSize: '0.7rem', fontWeight: 800, color: camp.effective_status === 'ACTIVE' ? 'var(--status-active)' : 'var(--status-warning)' }}>{camp.effective_status}</span>
                        </td>
                        <td onClick={() => fetchAdSets(camp.id, camp.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{camp.objective.replace(/_/g, ' ')}</div>
                        </td>
                        <td onClick={() => fetchAdSets(camp.id, camp.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)', textAlign: 'right' }}>
                          <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{formatCurrency(insight?.spend || 0)}</span>
                        </td>
                  ...
                  {viewLevel === 'AD_SETS' && metaData.adsets.map(adset => {
                    ...
                    return (
                      <tr key={adset.id} className="hover-row" style={{ cursor: 'pointer' }}>
                        <td onClick={() => fetchAds(adset.id, adset.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '1rem', marginBottom: '2px' }}>{adset.name}</div>
                          <div style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)' }}>ID: {adset.id}</div>
                        </td>
                        <td onClick={() => fetchAds(adset.id, adset.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          <span style={{ fontSize: '0.7rem', fontWeight: 800, color: adset.effective_status === 'ACTIVE' ? 'var(--status-active)' : 'var(--status-warning)' }}>{adset.effective_status}</span>
                        </td>
                        <td onClick={() => fetchAds(adset.id, adset.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          <div style={{ fontWeight: 600, fontSize: '0.9rem', color: '#60A5FA' }}>{formatBudget(displayBudget)}</div>
                          <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)' }}>{isCBO ? 'Advantage Campaign Budget' : `Budget ${adset.bid_strategy?.replace(/_/g, ' ') || 'Lowest Cost'}`}</div>
                        </td>
                        <td onClick={() => fetchAds(adset.id, adset.name)} style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid var(--border-color)', textAlign: 'right' }}>
                          <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{formatCurrency(insight?.spend || 0)}</span>
                        </td>
                        <td onClick={() => fetchAds(adset.id, adset.name)} style={{ padding: '1.4rem 1.8rem', textAlign: 'right', borderBottom: '1px solid var(--border-color)' }}>
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-tertiary)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
                        </td>
                      </tr>
                    );
                  })}
                  ...
                  {viewLevel === 'ADS' && metaData.ads.map(ad => {
                    ...
                    return (
                      <tr key={ad.id} className="hover-row" onClick={() => { setSelectedAd(ad); setIsModalOpen(true); }} style={{ cursor: 'pointer' }}>
                        <td style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                            {ad.creative?.thumbnail_url ? (
                              <img 
                                src={ad.creative.thumbnail_url} 
                                alt="Creative" 
                                style={{ width: '80px', height: '80px', borderRadius: '12px', objectFit: 'cover', border: '1px solid var(--border-color)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} 
                              />
                            ) : (
                              <div style={{ width: '80px', height: '80px', borderRadius: '12px', background: 'var(--bg-input)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                 <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--text-tertiary)" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                              </div>
                            )}
                            <div>
                              <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '0.95rem', marginBottom: '4px' }}>{ad.name}</div>
                              <div style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)' }}>Ad ID: {ad.id}</div>
                            </div>
                          </div>
                        </td>
                        <td style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                           <span style={{ fontSize: '0.7rem', fontWeight: 800, color: ad.effective_status === 'ACTIVE' ? 'var(--status-active)' : 'var(--status-warning)' }}>{ad.effective_status}</span>
                        </td>
                        <td style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                          {insight ? (
                            <>
                              <div style={{ fontWeight: 800, fontSize: '1.1rem', color: parseFloat(insight.purchase_roas_val) > 2 ? 'var(--status-active)' : 'var(--text-primary)' }}>
                                {parseFloat(insight.purchase_roas_val || '0').toFixed(2)}x ROAS
                              </div>
                              <div style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)' }}>Spend: {formatCurrency(insight.spend)}</div>
                            </>
                          ) : (
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', fontStyle: 'italic' }}>No Insights...</div>
                          )}
                        </td>
                        {viewLevel === 'ADS' && (
                          <>
                            <td style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                              <div style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{formatNumber(insight?.reach || 0)}</div>
                              <div style={{ fontSize: '0.6rem', color: 'var(--text-tertiary)' }}>{formatNumber(insight?.impressions || 0)} Imp.</div>
                            </td>
                            <td style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                              <div style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{parseFloat(insight?.frequency || '0').toFixed(2)}</div>
                            </td>
                            <td style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid var(--border-color)' }}>
                              <div style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{formatNumber(insight?.inline_link_clicks || 0)}</div>
                              <div style={{ fontSize: '0.6rem', color: 'var(--text-tertiary)' }}>{insight?.ctr ? `${parseFloat(insight.ctr).toFixed(2)}% CTR` : '0% CTR'}</div>
                            </td>
                          </>
                        )}
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '2.5rem' }}>
              <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                ...
                />
                <div>
                  <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>{selectedAd.name}</h2>
                  <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
                    <span style={{ background: 'var(--status-active-bg)', color: 'var(--status-active)', fontSize: '0.7rem', fontWeight: 800, padding: '4px 10px', borderRadius: '8px' }}>
                      {selectedAd.status}
                    </span>
                    <span style={{ color: 'var(--text-tertiary)', fontSize: '0.75rem' }}>ID: {selectedAd.id}</span>
                  </div>
                </div>
              </div>
              <button 
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
              ...
              <div className="modal-section" style={{ background: 'var(--bg-input)', padding: '1.5rem', borderRadius: '20px', border: '1px solid var(--border-color)' }}>
                <h3 style={{ fontSize: '0.8rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', marginBottom: '1.5rem', letterSpacing: '1px' }}>Core Financials</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>SPEND</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatCurrency(getInsightForId(selectedAd.id)?.spend || 0)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>ROAS</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--status-active)' }}>{parseFloat(getInsightForId(selectedAd.id)?.purchase_roas_val || '0').toFixed(2)}x</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>CPC</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatCurrency(getInsightForId(selectedAd.id)?.cpc || 0)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>CPM</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatCurrency(getInsightForId(selectedAd.id)?.cpm || 0)}</div>
                  </div>
                </div>
              </div>
              ...
              {/* Quality & Visibility */}
              <div className="modal-section" style={{ background: 'var(--bg-input)', padding: '1.5rem', borderRadius: '20px', border: '1px solid var(--border-color)' }}>
                <h3 style={{ fontSize: '0.8rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', marginBottom: '1.5rem', letterSpacing: '1px' }}>Quality & Delivery</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>FREQUENCY</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{parseFloat(getInsightForId(selectedAd.id)?.frequency || '0').toFixed(2)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>REACH</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatNumber(getInsightForId(selectedAd.id)?.reach || 0)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>QUALITY RANK</div>
                    <div style={{ fontSize: '0.8rem', fontWeight: 800, color: 'var(--accent-color)' }}>{getInsightForId(selectedAd.id)?.quality_ranking?.replace(/_/g, ' ') || 'AVERAGE'}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>CONV. RANK</div>
                    <div style={{ fontSize: '0.8rem', fontWeight: 800, color: 'var(--accent-color)' }}>{getInsightForId(selectedAd.id)?.conversion_rate_ranking?.replace(/_/g, ' ') || 'AVERAGE'}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="modal-section" style={{ background: 'var(--bg-input)', padding: '1.5rem', borderRadius: '20px', border: '1px solid var(--border-color)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatNumber(getInsightForId(selectedAd.id)?.inline_link_clicks || 0)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>CTR</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{parseFloat(getInsightForId(selectedAd.id)?.ctr || '0').toFixed(2)}%</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>REACTIONS</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatNumber(getInsightForId(selectedAd.id)?.post_reaction_val || 0)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', marginBottom: '4px' }}>COMMENTS</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{formatNumber(getInsightForId(selectedAd.id)?.post_comment_val || 0)}</div>
                  </div>
                </div>
              </div>
              ...
              {/* Video Funnel */}
              <div className="modal-section" style={{ background: 'var(--bg-input)', padding: '1.5rem', borderRadius: '20px', border: '1px solid var(--border-color)' }}>
                <h3 style={{ fontSize: '0.8rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', marginBottom: '1.5rem', letterSpacing: '1px' }}>Video Funnel</h3>
                <div style={{ borderLeft: '2px solid var(--border-color)', paddingLeft: '1.5rem' }}>
                  <div style={{ marginBottom: '1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--text-tertiary)', marginBottom: '5px' }}>
                      <span>25% Watched</span>
                      <span style={{ fontWeight: 800, color: 'var(--text-primary)' }}>{formatNumber(getInsightForId(selectedAd.id)?.video_p25_val || 0)}</span>
                    </div>
                    <div style={{ height: '4px', background: 'var(--border-color)', borderRadius: '2px' }}>
                      <div style={{ height: '100%', width: '100%', background: 'var(--accent-color)', borderRadius: '2px' }}></div>
                    </div>
                  </div>
                  <div style={{ marginBottom: '1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--text-tertiary)', marginBottom: '5px' }}>
                      <span>50% Watched</span>
                      <span style={{ fontWeight: 800, color: 'var(--text-primary)' }}>{formatNumber(getInsightForId(selectedAd.id)?.video_p50_val || 0)}</span>
                    </div>
                    <div style={{ height: '4px', background: 'var(--border-color)', borderRadius: '2px' }}>
                      <div style={{ height: '100%', width: '50%', background: 'var(--accent-color)', borderRadius: '2px' }}></div>
                    </div>
                  </div>
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--text-tertiary)', marginBottom: '5px' }}>
                      <span>100% Watched</span>
                      <span style={{ fontWeight: 800, color: 'var(--text-primary)' }}>{formatNumber(getInsightForId(selectedAd.id)?.video_p100_val || 0)}</span>
                    </div>
                    <div style={{ height: '4px', background: 'var(--border-color)', borderRadius: '2px' }}>
                      <div style={{ height: '100%', width: '20%', background: 'var(--accent-color)', borderRadius: '2px' }}></div>
                    </div>
                  </div>
                </div>
              </div>
