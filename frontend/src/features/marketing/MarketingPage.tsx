...
export function MarketingPage({ token, onUnauthorized }: MarketingPageProps) {
  ...
  return (
    <section className="workspace-page marketing-page" aria-labelledby="marketing-heading">
      <header className="workspace-page-header"><div><p className="eyebrow">Engagement / Marketing</p><h2 id="marketing-heading">Marketing</h2><p>Review paid marketing performance by account, campaign, ad set, and creative.</p></div><button className="secondary-button" type="button" onClick={() => void (level === 'campaigns' ? loadOverview() : level === 'adsets' && campaignContext ? loadAdsets({ id: campaignContext.id, name: campaignContext.name }) : adsetContext ? loadAds({ id: adsetContext.id, name: adsetContext.name }) : loadOverview())} disabled={isLoading}><RefreshCw size={15} className={isLoading ? 'spin' : undefined} aria-hidden="true" /> Refresh</button></header>
