                  ...
                  <td style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border-color-light)', textAlign: 'right' }}>
                    {userRole === 'admin' ? (
                      <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end', alignItems: 'center' }}>
                        <button type="button" aria-label={tr.enabled ? "Pause" : "Resume"} onClick={() => handleToggle(tr.id, tr.enabled)} title={tr.enabled ? "Pause" : "Resume"} style={{ background: 'none', border: 'none', cursor: 'pointer', color: tr.enabled ? 'var(--status-active)' : 'var(--text-tertiary)' }} className="action-btn">
                        ...
                        <button type="button" aria-label="Remove" onClick={() => handleDelete(tr.id)} title="Remove" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--status-error)' }} className="action-btn">
