...
func (r *gormOrderRepository) syncInventoryDeltas(tx *gorm.DB, order *entity.Order, oldItems []entity.LineItem) ([]int, error) {
	...
	var mappings []inventoryEntity.InventoryMapping
...
func (r *gormOrderRepository) Upsert(order entity.Order) ([]int, error) {
	...
	if order.CustomerPhone != nil {
		normalized := util.NormalizePhone(*order.CustomerPhone)
	...
	err := r.db.Transaction(func(tx *gorm.DB) error {
		...
		if strings.ToLower(strings.TrimSpace(util.DerefStr(order.DeliveryStatus))) == "delivered" && order.DeliveredAt == nil {
			now := time.Now()
...
func (r *gormOrderRepository) UpsertBatch(orders []entity.Order) ([]int, error) {
	...
	err := r.db.Transaction(func(tx *gorm.DB) error {
		...
		for i := range orders {
			...
			// Stamp delivered_at if status transition detected
			if strings.ToLower(strings.TrimSpace(util.DerefStr(orders[i].DeliveryStatus))) == "delivered" && orders[i].DeliveredAt == nil {
				now := time.Now()
