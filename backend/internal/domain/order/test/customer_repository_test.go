...
func (s *CustomerRepositoryTestSuite) SetupSuite() {
	...
	s.db = db
	s.repo = repository.NewCustomerRepository(db)
}
...
func (s *CustomerRepositoryTestSuite) TestUpsertAndGet() {
	...
	customer := &entity.Customer{
		PhoneNumber: phone,
		FirstName:   util.StrPtr("Alice"),
		LastName:    util.StrPtr("Doe"),
		Email:       util.StrPtr("alice@example.com"),
		TotalOrders: 5,
		TotalSpent:  500.0,
	}
...
func (s *CustomerRepositoryTestSuite) TestListWithFilters() {
	ctx := context.Background()
	s.repo.UpsertByPhone(ctx, &entity.Customer{PhoneNumber: "111", FirstName: util.StrPtr("Alice"), City: util.StrPtr("Delhi"), TotalSpent: 100})
	s.repo.UpsertByPhone(ctx, &entity.Customer{PhoneNumber: "222", FirstName: util.StrPtr("Bob"), City: util.StrPtr("Mumbai"), TotalSpent: 500})
...
func (s *CustomerRepositoryTestSuite) TestListSQLInjection() {
	ctx := context.Background()
	s.repo.UpsertByPhone(ctx, &entity.Customer{PhoneNumber: "111", FirstName: util.StrPtr("Alice")})
