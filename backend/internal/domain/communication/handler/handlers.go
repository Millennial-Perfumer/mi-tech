...
type AutomationHandler struct {
	templatesService *service.TemplatesService
	messagesService  *service.MessagesService
	mappingService   *service.WebhookMappingService
	orderService     *orderServicePkg.OrderService
	customerService  *orderServicePkg.CustomerService
	settings         *config.SettingsProvider
	agentService     *service.AgentService
}

func NewAutomationHandler(tService *service.TemplatesService, mService *service.MessagesService, mappingService *service.WebhookMappingService, orderService *orderServicePkg.OrderService, customerService *orderServicePkg.CustomerService, settings *config.SettingsProvider, agentService *service.AgentService) *AutomationHandler {
...
// @Accept json
// @Produce json
// @Param body body dto.CreateTemplateRequest true "Template data"
...
// @Success 200 {array} entity.AutomationTemplate
...
// @Security Bearer
// @Produce json
// @Success 200 {array} entity.Trigger
...
// @Accept json
// @Produce json
// @Param body body entity.AutomationTemplate true "Updated mapping"
// @Success 200 "OK"
...
func (h *AutomationHandler) UpdateTemplate(w http.ResponseWriter, r *http.Request) {
	...
	var req entity.AutomationTemplate
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
...
func (h *AutomationHandler) UploadChatMedia(w http.ResponseWriter, r *http.Request) {
	...
	id, err := h.messagesService.GetMetaClient().UploadWhatsAppMedia(fileBytes, filename, mimeType)
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to upload to Meta: %v", err), http.StatusInternalServerError)
		return
	}
...
func (h *AutomationHandler) SendChatMedia(w http.ResponseWriter, r *http.Request) {
	...
	msgID, err := h.messagesService.GetMetaClient().SendMediaMessage(req.PhoneNumber, req.MediaID, req.Type, req.Caption)
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to send media via Meta: %v", err), http.StatusInternalServerError)
		return
	}

	convID, err := h.messagesService.GetRepo().UpsertConversation(req.PhoneNumber, "", displayText)
	if err != nil {
	...
	_, err = h.messagesService.GetRepo().SaveChatMessage(chatMsg)
	if err != nil {
...
func (h *AutomationHandler) SendBulkMarketing(w http.ResponseWriter, r *http.Request) {
	...
	template, err := h.templatesService.GetRepo().GetTemplateByID(req.TemplateID)
...
func (h *AutomationHandler) CreateEvent(w http.ResponseWriter, r *http.Request) {
	var req entity.AutomationEvent
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
