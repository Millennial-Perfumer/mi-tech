...
func (s *WebhookMappingService) ExecuteMapping(storeID, topic string, order entity.Order) error {
	...
	tx, err := s.messagesService.repo.GetDB().Begin()
	if err != nil {
...
func (s *WebhookMappingService) ResolveVariable(field string, order entity.Order, totals *service.InvoiceTotals) string {
	...
	switch field {
	case "customer_name":
		name := util.DerefStr(order.CustomerFirstName)
		if name == "" {
			name = util.DerefStr(order.CustomerName)
		}
		if name == "" {
			return "Customer"
		}
	...
	case "order_subtotal":
		return fmt.Sprintf("%.2f", util.DerefFloat64(order.SubtotalPrice))
	case "order_discount":
		return fmt.Sprintf("%.2f", order.TotalDiscount)
	case "order_tax":
		return fmt.Sprintf("%.2f", util.DerefFloat64(order.TotalTax))
	case "currency":
		return util.DerefStr(order.Currency)
	case "tracking_link":
		return util.DerefStr(order.TrackingUrl)
	case "tracking_number":
		return util.DerefStr(order.TrackingNumber)
	case "shipping_company":
		return util.DerefStr(order.ShippingCompany)
	case "customer_city":
		return util.DerefStr(order.CustomerCity)
	case "customer_state":
		return util.DerefStr(order.CustomerState)
	case "customer_country":
		return util.DerefStr(order.CustomerCountry)
	case "customer_zip":
		return util.DerefStr(order.CustomerZip)
	case "customer_address1":
		return util.DerefStr(order.CustomerAddress1)
	case "customer_address2":
		return util.DerefStr(order.CustomerAddress2)
	case "customer_email":
		return util.DerefStr(order.CustomerEmail)
	case "customer_phone":
		return util.DerefStr(order.CustomerPhone)
...
func (s *WebhookMappingService) executeWithTemplate(storeID string, template *commEntity.AutomationTemplate, order entity.Order, topic string) error {
...
func resolveCustomerVariable(field string, customer *entity.Customer) string {
	switch field {
	case "customer_name":
		name := util.DerefStr(customer.FirstName)
		if name == "" {
			name = util.DerefStr(customer.LastName)
		}
		if name == "" {
			return "Customer"
		}
		return name
	case "customer_city":
		return util.DerefStr(customer.City)
	case "customer_state":
		return util.DerefStr(customer.State)
	case "customer_country":
		return util.DerefStr(customer.Country)
	case "customer_zip":
		return util.DerefStr(customer.ZipCode)
	case "customer_address1":
		return util.DerefStr(customer.Address1)
	case "customer_address2":
		return util.DerefStr(customer.Address2)
	case "customer_email":
		return util.DerefStr(customer.Email)
	case "customer_phone":
...
func (s *WebhookMappingService) ExecuteMarketingSend(storeID string, template *commEntity.AutomationTemplate, customer *entity.Customer) error {
	var mappings map[string]string
	if template.VariableMappings != nil {
		json.Unmarshal(*template.VariableMappings, &mappings)
	}
...
func (s *WebhookMappingService) GenerateFeedbackURL(order entity.Order) string {
	...
	q.Set("phone", SanitizePhoneNumber(util.DerefStr(order.CustomerPhone)))
	u.RawQuery = q.Encode()
