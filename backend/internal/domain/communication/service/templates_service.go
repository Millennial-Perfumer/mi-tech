...
func NewTemplatesService(repo repository.TemplatesRepository, settings *config.SettingsProvider) *TemplatesService {
...
func (s *TemplatesService) CreateTemplate(storeID string, req dto.CreateTemplateRequest) (int, error) {
	...
	template := entity.AutomationTemplate{
		StoreID:        storeID,
...
func (s *TemplatesService) mapToMetaComponents(req dto.CreateTemplateRequest) ([]map[string]interface{}, error) {
...
func (s *TemplatesService) GetTriggers(storeID string) ([]entity.Trigger, error) {
...
func (s *TemplatesService) GetTemplates(storeID string, startDate, endDate *time.Time) ([]entity.AutomationTemplate, error) {
...
func (s *TemplatesService) FetchRemoteTemplate(templateName string) (*dto.CreateTemplateRequest, error) {
	...
	for _, comp := range remote.Components {
		...
		case "HEADER":
			...
			req.Header = &entity.TemplateHeader{
...
func (s *TemplatesService) SyncAllTemplates(storeID string) error {
	...
	for _, remote := range remoteTemplates {
		...
		localTpl := entity.AutomationTemplate{
			StoreID:        storeID,
		...
		var header *entity.TemplateHeader
		var buttons []entity.TemplateButton
		var footer string
		...
		for _, comp := range remote.Components {
			...
			case "HEADER":
				format, _ := comp["format"].(string)
				header = &entity.TemplateHeader{Type: strings.ToLower(format)}
			...
			case "BUTTONS":
				if btns, ok := comp["buttons"].([]interface{}); ok {
					for _, b := range btns {
						...
						newBtn := entity.TemplateButton{Text: bText}
						switch bType {
						case "QUICK_REPLY":
							newBtn.Type = "custom"
						case "URL":
...
func (s *TemplatesService) GetEvents() ([]entity.AutomationEvent, error) {
	return s.repo.GetEvents()
}

func (s *TemplatesService) SaveEvent(e entity.AutomationEvent) error {
