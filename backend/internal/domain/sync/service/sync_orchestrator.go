...
func (s *SyncOrchestrator) GlobalSyncBatch(ctx context.Context, itemIDs []int, sourcePlatform string) error {
	...
	// Discover location ID once per batch if needed, but ONLY if we actually have Shopify mappings
	var shopifyLocationID string
	var needsShopifySync bool

	// First pass to check if we need Shopify sync
	for _, item := range items {
		for _, m := range item.Mappings {
			if m.Platform != sourcePlatform && m.Platform == "shopify" && m.ExternalVariantID != nil {
	...
	for _, item := range items {
		...
		for _, m := range item.Mappings {
			...
			g.Go(func() error {
				switch m.Platform {
				case "shopify":
					if m.ExternalVariantID != nil && shopifyLocationID != "" {
						log.Printf("SyncOrchestrator: Pushing stock update to Shopify for inventory item %s", *m.ExternalVariantID)
