...
func (p *AmazonOrderPoller) SyncOrders(ctx context.Context, start, end *time.Time) {
	...
	// Pre-fetch order items concurrently to avoid O(N) network latency bottleneck
	orderItemsMap := make(map[string][]map[string]interface{})
	var itemsMutex sync.Mutex
	eg, _ := errgroup.WithContext(ctx) // Don't need context cancellation for this
	eg.SetLimit(5)
	...
	for _, ao := range amazonOrders {
		...
		items, ok := orderItemsMap[amazonOrderID]
		if !ok {
			slog.Error("AmazonOrderPoller: Failed to fetch items for order (missing from pre-fetch map)", "orderID", amazonOrderID)
			continue
		}
