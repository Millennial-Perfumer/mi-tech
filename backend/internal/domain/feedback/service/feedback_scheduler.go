...
func (s *FeedbackScheduler) executeFeedbackScanAndSend(ctx context.Context) {
	log.Println("FeedbackScheduler: Starting automated scan for feedback concerns...")

	delayMins := s.settingsProvider.GetFeedbackAutomationDelayMinutes()
	orders, err := s.feedbackService.GetOrdersForFeedback(delayMins)
	if err != nil {
	...
	var eligibleOrders []orderEntity.Order
	for _, order := range orders {
		if order.DeliveredAt == nil {
			continue
		}
		customerPhone := util.DerefStr(order.CustomerPhone)
		if customerPhone == "" {
			continue
		}
	...
	for _, o := range eligibleOrders {
		...
		wg.Add(1)
		go func(order orderEntity.Order) {
			defer wg.Done()
			...
			fullOrder, err := s.feedbackService.GetOrderEntity(order.ID)
			if err != nil {
			...
			_ = s.feedbackService.UpdateFeedbackStatus(order.ID, 2)
