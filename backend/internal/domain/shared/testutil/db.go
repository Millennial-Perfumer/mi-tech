...
func SetupTestDB() (*gorm.DB, error) {
	...
	migrationsPath := filepath.Join(root, "internal/domain/shared/database/migrations")
...
func runMigrations(db *gorm.DB, migrationsDir string) error {
	// 1. Change working directory temporarily to the backend root
	// because RunMigrations expects "internal/domain/shared/database/migrations" relative path
	_, b, _, _ := runtime.Caller(0)
	basepath := filepath.Dir(b)
	root := filepath.Join(basepath, "..", "..", "..", "..")
