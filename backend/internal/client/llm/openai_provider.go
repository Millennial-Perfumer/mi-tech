...
func (p *OpenAIProvider) Chat(ctx context.Context, messages []ChatMessage, tools []ToolDef) (*ChatResponse, error) {
...
func (p *OpenAIProvider) StreamChat(ctx context.Context, messages []ChatMessage, tools []ToolDef) (<-chan StreamChunk, error) {
	...
	go func() {
		...
		if resp.StatusCode != http.StatusOK {
			body, _ := io.ReadAll(resp.Body)
			ch <- StreamChunk{Error: fmt.Errorf("openai error (%d): %s", resp.StatusCode, string(body))}
			return
		}
