...
func (s *SyncService) Sync() (int, error) {
	...
	err := s.db.QueryRow("SELECT MAX(updated_at) FROM shopify_orders").Scan(&lastSync)
	...
	if len(shopifyOrders) == 0 {
		log.Println("No new or updated orders found from Shopify")
		return 0, nil
	}
	...
	// We use ON CONFLICT (id) DO UPDATE to seamlessly handle existing vs new orders
	orderQuery := `
	INSERT INTO shopify_orders (
		id, order_number, total_price, subtotal_price, total_tax, created_at, updated_at,
	...
	itemQuery := `
	INSERT INTO shopify_order_line_items (id, order_id, title, sku, hs_code, quantity, price, discount)
