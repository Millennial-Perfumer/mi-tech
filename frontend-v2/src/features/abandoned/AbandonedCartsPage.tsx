...
export function AbandonedCartsPage({ token, onUnauthorized }: AbandonedCartsPageProps) {
  ...
  return (
    <section className="workspace-page abandoned-page" aria-labelledby="abandoned-heading">
      <header className="workspace-page-header"><div><p className="eyebrow">Engagement / Recovery</p><h2 id="abandoned-heading">Recover the sale, not the noise.</h2><p>See where checkouts drop off, send a recovery message when it helps, and keep the recovery funnel measurable.</p></div><button className="secondary-button" type="button" onClick={() => activeTab === 'list' ? void loadList() : void loadAnalytics()} disabled={isLoading}><RefreshCw size={15} className={isLoading ? 'spin' : undefined} aria-hidden="true" /> Refresh</button></header>
