...
export function FeedbackPage({ token, onUnauthorized }: FeedbackPageProps) {
  ...
  const [judgeMeEmail, setJudgeMeEmail] = useState(DEFAULT_REVIEWER_EMAIL)
  ...
  const openFeedback = (feedback: Feedback) => {
    ...
    setJudgeMeEmail(DEFAULT_REVIEWER_EMAIL)
    setNotice('')
  }
  ...
  return (
    <section className="workspace-page feedback-page" aria-labelledby="feedback-heading">
      ...
      {selectedFeedback && <div className="modal-scrim" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedFeedback(null) }}><div className="modal-card feedback-modal" role="dialog" aria-modal="true" aria-labelledby="feedback-detail-heading"><div className="modal-heading feedback-modal-heading"><div><p className="eyebrow">Customer feedback · Order {selectedFeedback.order_number}</p><h2 id="feedback-detail-heading">{selectedFeedback.customer_name || 'Anonymous customer'}</h2></div><button className="icon-button" type="button" aria-label="Close review" onClick={() => setSelectedFeedback(null)}><X size={19} aria-hidden="true" /></button></div><div className="feedback-detail-rating"><Stars rating={selectedFeedback.rating} /><strong>{selectedFeedback.rating}/5</strong><span>{formatDate(selectedFeedback.created_at)}</span></div><section className="feedback-review-section"><div className="feedback-section-heading"><span className="metric-label">Customer review</span>{selectedFeedback.judgeme_posted && <span className="status-pill status-pill-success"><Check size={13} aria-hidden="true" /> Posted</span>}</div><blockquote>{selectedFeedback.message || 'No written comment.'}</blockquote></section><div className="feedback-action-row"><button className="secondary-button" type="button" onClick={() => void copyText(selectedFeedback.message)}><Clipboard size={14} aria-hidden="true" /> Copy review</button>{selectedFeedback.customer_phone && <a className="secondary-button" href={`https://api.whatsapp.com/send?phone=${selectedFeedback.customer_phone.replace(/\D/g, '')}&text=${encodeURIComponent(selectedFeedback.message)}`} target="_blank" rel="noreferrer"><MessageSquare size={14} aria-hidden="true" /> Open WhatsApp</a>}</div><section className="feedback-note-section"><div className="feedback-section-heading"><div><span className="metric-label">Internal note</span><p>Keep useful context for your team.</p></div></div><label className="form-field"><span className="sr-only">Internal note</span><textarea rows={3} value={adminComment} onChange={(event) => setAdminComment(event.target.value)} placeholder="Add context for your team" /></label><div className="feedback-note-actions"><button className="secondary-button" type="button" onClick={() => void saveComment()} disabled={isWorking}><Save size={14} aria-hidden="true" /> Save note</button></div></section><div className="feedback-integrations"><div className="feedback-integration-card"><div className="feedback-section-heading"><div><span className="metric-label">Judge.me</span><p>{selectedFeedback.judgeme_posted ? 'This review has already been posted.' : 'Post this response as a product review.'}</p></div></div>{!selectedFeedback.judgeme_posted && <div className="feedback-email-row"><label className="feedback-email-field"><span>Email address</span><input aria-label="Judge.me email" type="email" value={judgeMeEmail} onChange={(event) => setJudgeMeEmail(event.target.value)} /></label><button className="primary-button" type="button" onClick={() => void postJudgeMe()} disabled={isWorking}><Send size={14} aria-hidden="true" /> Post</button></div>}</div><div className="feedback-integration-card"><div className="feedback-section-heading"><div><span className="metric-label">Google review</span><p>{selectedFeedback.google_review_requested ? 'Request already recorded.' : 'Record a Google review follow-up.'}</p></div></div>{!selectedFeedback.google_review_requested && <button className="secondary-button feedback-integration-action" type="button" onClick={() => void requestGoogleReview()} disabled={isWorking}>Record request</button>}</div></div></div></div>}
